...
class HD44780(object):
    ...
    def __init__(self):
        ...
        GPIO.setup(self.SW1, GPIO.IN)  # SW1
        GPIO.setup(self.SW2, GPIO.IN)  # SW1
...
def main():
    ...
    while 1:
        ...
        if GPIO.input(17):
            lcd.string(now.strftime("%Y-%m-%d"), 1)
        else:
            lcd.string(get_ip_address('eth0'), 1)
        if GPIO.input(18):
            lcd.string(now.strftime("%H:%M:%S"), 2)
        else:
            lcd.string(u"てすとてすと", 2)
        time.sleep(0.2)
